...
{if(!q||!q.length)return null;var res={};if(q[0]=='?')q=q.substring(1);var p=q.split('&');for(var i=0;i<p.length;i++){var x=p[i].split('=');if(x.length==1){res[dec(x[0])]=null;continue;}
...
{var tmp=[];for(var name in this.variables){var t;if(this.variables[name]!=null){if(typeof this.variables[name]!=='string'){var tt=[];for(var i=0;i<this.variables[name].length;i++)
tt.push(enc(name)+'='+enc(this.variables[name][i]));t=tt.join('&');}
else
